...
export function useMapElements(eventId, planId) {
    const elements = ref([])
    const saving = ref(false)
    const error = ref(null)

    async function load() {
        const { data } = await axios.get(`/api/plans/${planId}/elements`)
    ...
    <div class="h-screen flex flex-col">
        ...
        <div class="flex items-center gap-3 px-4 py-2 border-b bg-white shrink-0 overflow-x-auto">
            ...
            <button
                ...
                title="Undo (Ctrl+Z)"
            >↩ Undo</button>
            <span v-if="saving" class="text-xs text-gray-400 ml-auto shrink-0">Saving…</span>
        </div>
